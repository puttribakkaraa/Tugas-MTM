...
<head>
    ...
    <style>
        ...
        /* Konsistensi Lebar Kolom Tanggal */
    ...
    <div class="bg-white p-6 rounded shadow-lg">
        ...
        <div class="mb-4 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-2">
                <form action="{{ route('absensi.index') }}" method="GET" class="flex gap-1">
...
@endif
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        @foreach($dateRange as $date)
                            @php
                                ...
                                if($attendance && $date->isToday()) { $totalHadirHariIni++; }
                            @endphp
                            <td class="{{ $date->isWeekend() ? 'bg-sunday' : '' }} p-0">
                                @if($attendance)
                                    <div class="flex flex-col items-center justify-center py-1">
                                        <span class="font-bold">
                                            {{ $attendance->status == 'Hadir' ? '✔' : ($attendance->status == 'Sakit' ? '●' : '▲') }}
                                        </span>
                        ...
                        <td class="sticky-col-right">
                            <div class="flex items-center justify-center gap-1 bg-[#f8fafc] py-1">
                                <button onclick="openEditModal({{ json_encode($emp) }})" class="btn-icon btn-edit-icon" title="Edit">
                                    <i class="fas fa-pen-to-square text-xs"></i>
                                </button>
                                
                                <form action="/absensi/{{ $emp->id }}" method="POST" onsubmit="return confirm('Hapus data karyawan {{ $emp->name }}?')">
                                    @csrf 
                                    @method('DELETE')
    ...
    <div id="modalEdit" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <form id="formEdit" method="POST" class="p-6 space-y-4">
                @csrf 
                @method('PUT')
                <div><label class="block text-sm font-semibold text-gray-500">NPK (Tidak bisa diubah)</label>
                    <input type="text" name="npk" id="edit_npk" readonly class="w-full mt-1 p-2 border rounded-lg bg-gray-100 cursor-not-allowed">
                </div>
    ...
    <script>
        ...
        function openEditModal(emp) {
            // URL Edit disesuaikan (biasanya /absensi/{id})
